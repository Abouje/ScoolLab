...
import torch
import transformers
from agentenv.controller import Agent, ChatMLTemplate
...
class TrainingArguments:
    ...
    inference_file: str = field(
        default="/raid/data/datasets/AgentTraj-L/webshop_train.json", metadata={"help": "Inference dataset."}
    )
    test_file: str = field(default="/raid/data/datasets/AgentEval/webshop_test.json", metadata={"help": "Test dataset."})
    # model path
    model_train_path: str = field(
        default="/raid/data/models/Qwen3-8B",
...
def main():
    ...
    trainer = BCTrainer(
        Agent(model, tokenizer, chat_template=ChatMLTemplate()),
