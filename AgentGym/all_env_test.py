...
timestamp = user_time.strftime("%Y-%m-%d %H:%M")

output_dir = "/raid/zyj/AgentGym/eval_results"
...
for task, task_idxs in zip(evaluator.tasks, idxs_list):
    ...
    for start in range(0, total, batch_size):
        ...
        print(f"[{task_name}] progress: {done}/{total} (last_idx={batch[-1]})", flush=True)
    all_exps.extend(task_exps)
    output_file = os.path.join(output_dir, f"{task_name}_{timestamp}.json")
    with open(output_file, "w") as f:
        json.dump(task_exps, f, ensure_ascii=False, indent=4)
